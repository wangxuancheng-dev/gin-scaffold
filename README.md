...
- 支持 cron 示例任务（`robfig/cron/v3`）：
  ...
  - 任务 `command` 支持 artisan 前缀：如 `artisan ping`、`artisan report:daily --date=2026-04-16`（与 `cmd/artisan run` 使用同一命令注册体系）
- 如需临时覆盖可显式传 `--dsn`
...
> 新生成命令位于 `internal/console/commands/*.go`，实现后重新编译即可使用。
