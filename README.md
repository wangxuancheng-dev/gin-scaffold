...
全量 `go test ./...` 与覆盖率会按 CPU 核数并行跑很多包，内存/CPU 容易打满；本机卡顿时可先设并行上限再跑质量脚本，例如 PowerShell：`$env:GOTEST_PARALLEL='2'; .\scripts\make.ps1 -Target quality`；Bash：`GOTEST_PARALLEL=2 bash scripts/quality.sh`。另：`golangci-lint ./...` 与 `go test -race` 也明显吃资源，尽量单独跑、避免与全量测试叠开。
