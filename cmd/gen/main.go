...
func runCRUD(opt crudOptions) error {
	...
	files := map[string]string{
		...
		filepath.Join("internal", "api", "request", "admin", moduleSnake+"_request.go"): requestTemplate(modelName, parsedFields),
		filepath.Join("internal", "api", "handler", "admin", moduleSnake+"_handler.go"): adminHandlerTemplate(modelName, serviceName, parsedFields),
		filepath.Join("internal", "routes", "adminroutes", moduleSnake+"_router.go"):    adminRouteTemplate(modelName, moduleSnake),
	}
...
func wireRouterOptions(modelName string) error {
	path := filepath.Join("internal", "routes", "router.go")
...
func wireAPIRouter(modelName string) error {
	path := filepath.Join("internal", "routes", "api_router.go")
	data, err := os.ReadFile(path)
	if err != nil {
		return err
	}
...
func wireAdminRouter(modelName string) error {
	path := filepath.Join("internal", "routes", "adminroutes", "register.go")
	data, err := os.ReadFile(path)
	if err != nil {
