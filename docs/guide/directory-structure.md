...
| `internal/config/` | Viper 加载、环境变量绑定、`Validate()` fail-fast |
| `internal/routes/` | Gin 引擎构建、全局中间件、`/api/v1` 注册 |
| `internal/routes/adminroutes/` | 管理端路由按模块拆分 |
| `internal/middleware/` | 可复用 HTTP 中间件 |
| `internal/api/handler/` | Controller：入参绑定、调用 service、统一错误出口 |
| `internal/api/request/` | 请求 DTO（`binding` 标签） |
| `internal/api/response/` | 统一 JSON 信封 |
...
| `tests/scenario`、`tests/integration` | 黑盒场景测试与集成测试 |
| `docs/` | VitePress 文档站 |
...
```text
internal/api/handler → internal/service/port → internal/service → internal/dao → internal/model
                         ↘ pkg/* 、 internal/config
```
...
- **封装边界**：应用代码统一在 `internal/*` 下，避免业务包对外暴露 import 面。
- **错误出口统一**：Handler 层优先使用 `internal/api/handler/error_helper.go` 的 `FailInvalidParam` / `FailNotFound` / `FailInternal`，避免散落 `response.FailHTTP(...)` 常量组合。
...
3. `internal/api/request` + `handler` + `response`（若有 VO）。
4. `internal/routes/adminroutes`（或 `client_router`）注册路由。
...
**Handler**：只做绑定、调 service、用统一错误出口与 `response.OK`（示例形态，与 `internal/api/handler/admin/*` 一致；省略 `import`）：
...
**路由**（管理端 + 权限串，见 `internal/routes/adminroutes/*.go`）：
