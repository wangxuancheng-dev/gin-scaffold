...
未在表中列出的嵌套键，仍可按 **`.` → `_` 且全大写** 规则自行推导并在运行时试验（以 `config/loader.go` 为准）。

### 临时覆盖示例

```bash
export DB_DSN='user:pass@tcp(127.0.0.1:3306)/gin_scaffold?charset=utf8mb4&parseTime=True'
export JWT_SECRET='dev-only-change-me'
go run ./cmd/server server --env dev
```

```powershell
$env:DB_DSN = "user:pass@tcp(127.0.0.1:3306)/gin_scaffold?charset=utf8mb4&parseTime=True"
